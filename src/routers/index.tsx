...
import {
    getRightsList,
    fetchUserById,
} from '../redux/actionCreators/rightsSlice'
...
const routes: Array<RouteType> = [
    ...
    {
        path: '/tourist',
        element:<Redirect to={'/tourist/news'} />, // 重定向
    },
    {
        path: '/tourist/news',
    ...
    {
        path: '/tourist/datail/:id',
        element: lazyLoad('News/datail'),
    },
    // 游客访问 - 路由结束
    {
        path: '/',
        //AuthComponent  路由拦截组件  验证 token
        element: <AuthComponent>{lazyLoad('SandBox')}</AuthComponent>,
        children: [
            {
                // path: '',
                ...
                children: [
                    {
                        ...
                        index: true, // 默认显示 待发布
                        element: (
                            <Redirect to={'/publish-manage/unpublished'} />
                        ), // 重定向
                    },
                    {
                        path: 'unpublished',
...
export default function MRouters() {
    ...
    const { rightsList } = useSelector(
        (state: RootState) => state.rightsSlice,
        shallowEqual
    )
    ...
    useEffect(() => {
        if (userInfo && userInfo?.id) {
            ...
            // console.log('获取rights-1',location)
            // 获取 url 信息
            let {pathname} = location;
            // 不存在 游客访问的 路由信息 重定向 (游客访问的路由 关键字 为 tourist)
